...
const Signup = () => {
  ...
  const handleSignupSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      await API.post(`${process.env.REACT_APP_API_URL}/api/register`, { name, mobile, email, password });
  ...
  const handleOtpSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      await API.post(`${process.env.REACT_APP_API_URL}/api/verify-otp`, { email, otp });
      setIsVerified(true);
      navigate('/login');
    } catch (error) {
    ...
    return (
      <motion.div initial={{opacity:0 }} 
        ...
        <div className="bg-white shadow-lg rounded-lg p-8 max-w-md w-full">
          ...
          <p className='text-sm text-gray-500 text-center mb-4'>An OTP has been sent to <strong>{email}</strong></p>
